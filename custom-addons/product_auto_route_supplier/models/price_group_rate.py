...
class PriceGroupRate(models.Model):
    _name = 'price.group.rate'
    _description = 'Price Group Discount Rate'
    _order = 'supplier_comp_code, group_name'
    ...
    supplier_comp_code = fields.Char(
        ...
        size=20,
        index=True,
        help='External API Supplier Code'
    )
    ...
    group_name = fields.Char(
        ...
        size=50,
        index=True,
        help='Price group identifier'
    )
    
    # Dropdown helper for common price groups
    group_name_selection = fields.Selection([
        ('PREMIUM', 'PREMIUM'),
        ('STANDARD', 'STANDARD'),
        ('GOLD', 'GOLD'),
        ('SILVER', 'SILVER'),
        ...
        ('WHOLESALE', 'WHOLESALE'),
        ('RETAIL', 'RETAIL'),
    ], string='Select Price Group', help='Select from common price groups (optional helper field)')
    ...
    rate = fields.Float(
        string='Multiplier Rate',
        required=True,
        digits=(5, 4),
        help='Multiplier rate (e.g., 0.75 means 25% discount from list price)'
    )
    ...
    discount_percent = fields.Float(
        ...
        store=True,
        digits=(5, 2),
        help='Discount percentage calculated from rate (e.g., rate 0.75 = 25% discount)'
    )
    
    valid_from = fields.Date(
        string='Valid From',
        help='Effective date of the rate'
    )
    
    valid_to = fields.Date(
        string='Valid To',
        help='End date of the rate'
    )
    
    active = fields.Boolean(
        string='Active',
        default=True,
        help='Only active records are used in price calculation'
    )
    
    note = fields.Text(
        string='Notes',
        help='Remarks or operational notes'
    )
    ...
    _sql_constraints = [
        ('unique_supplier_group_active',
         'UNIQUE(supplier_comp_code, group_name, active) WHERE active = true',
         'Only one active record per supplier and price group is allowed!'),
        ('rate_positive',
    ...
    def _compute_discount_percent(self):
        ...
        for record in self:
            if record.rate:
                # If rate is 0.75, discount is 25%
                record.discount_percent = (1.0 - record.rate) * 100.0
            else:
                record.discount_percent = 0.0

    @api.depends('supplier_comp_code', 'group_name', 'rate')
    def _compute_display_name_full(self):
        """Compute display name"""
        for record in self:
            record.display_name_full = (
                f"{record.supplier_comp_code} / {record.group_name} "
    ...
    @api.onchange('group_name_selection')
    def _onchange_group_name_selection(self):
        """Auto-fill group name when selected from dropdown"""
        if self.group_name_selection:
            self.group_name = self.group_name_selection
    ...
    @api.model
    def get_rate(self, supplier_comp_code, group_name, check_date=None):
        """
        Get price group rate
        
        Args:
            supplier_comp_code: Supplier company code
            group_name: Price group name
            check_date: Date to check validity (default: today)
            
        Returns:
            float: rate value or False if not found
        """
        if not supplier_comp_code or not group_name:
            return False
        
        if check_date is None:
            check_date = date.today()
        
        # Search with date validity
        domain = [
        ...
        if rate_record:
            if rate_record.valid_from and check_date < rate_record.valid_from:
                _logger.warning(
                    f"Price group rate not yet valid: {supplier_comp_code}/{group_name}"
                )
                return False
            if rate_record.valid_to and check_date > rate_record.valid_to:
                _logger.warning(
                    f"Price group rate expired: {supplier_comp_code}/{group_name}"
                )
                return False
            
            _logger.info(
                f"Price group rate found: {supplier_comp_code}/{group_name} = {rate_record.rate}"
            )
            return rate_record.rate
        
        _logger.debug(
            f"Price group rate not found: {supplier_comp_code}/{group_name}"
        )
        return False

    @api.model
    def get_discount_percent(self, supplier_comp_code, group_name, check_date=None):
        """
        Get discount percentage for price group
        ...
        """
        rate = self.get_rate(supplier_comp_code, group_name, check_date)
        if rate:
